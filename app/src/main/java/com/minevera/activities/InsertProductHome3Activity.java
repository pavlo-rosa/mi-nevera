 ...
 */
//ok
public class InsertProductHome3Activity extends AppCompatActivity implements View.OnClickListener, DateFragmentText.OnFragmentInteractionListener{
    private  final String TAG = "InsertProductHome3";
    ...
    private void checkIntent() {
        ...
        if(getIntent().getBooleanExtra("resultOK", false) == true){
//            Snackbar.make(mLayout, R.string.productFound, Snackbar.LENGTH_LONG).show();
            mLayoutBarcode.setVisibility(View.VISIBLE);

            myProduct.setNameProduct(getIntent().getStringExtra("resultNameProduct"));
            myProduct.setBarcode(getIntent().getLongExtra("resultBarcode",0000));
            myProduct.setImage(getIntent().getStringExtra("resultImage"));



            mProduct.setText(myProduct.getNameProduct());
            mProduct.setFocusable(false);
            mProduct.setEnabled(false);
            mBarcode.setText(myProduct.getBarcode().toString());
            printImage(myProduct);
            getIntent().removeExtra("resultOK");
        }
    }
    ...
    private void loadProductAutoCompleteTextView() {
        ...
        mProduct.addTextChangedListener(new TextWatcher() {
            ...
            private void autoSelectSpinnerKing(int i) {
